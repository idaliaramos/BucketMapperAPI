...
class DestinationService {
  ...
  async update(destinationId, attributes, authentication) {
    console.log(destinationId, attributes, authentication, 'kn the dest service')
    try {
      ...
      //fix
      //check
      console.log(record, 'record in dest service')
      if (!authenticatedUser) throw this._createPermissionDeniedError();
      const destination = await this._destinationRepository.update(
        destinationId,
        attributes
      );
      console.log(destination, 'dest in dest service')
