...
response.status(200).send('hello this is a test')
}
  async create(request, response, next) {
    console.log(request.body, 'req.body')
    try {
      // console.log(entityService, 'this is the service')
      const entity = await this._entityService.create(request.body);
      console.log(entity,  '<<<<<<<<<<this is the entity')
      if (!entity) {
        ...
        response
          .status(400)
          .send(
            " AHHHHHHHH error: please use unique email and provide name, email and password "
          );
      }
      response
        .sendStatus(201)
      ...
      response.status(400).send("whoops! server error in the entity controller");
    }
  }
  ...
  async update(request, response, next) {
    try {
      ...
      console.log(id, request.body, userId, '<<<<<<<<<<<<<<<<<<<<<<info to pass')
