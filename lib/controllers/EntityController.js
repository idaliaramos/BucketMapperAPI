...
class EntityController {
  ...
  async create(request, response, next) {
    console.log(request.body, 'this is the body in the controlloer');
    try {
      ...
      console.log(entity, 'this is the enity');
      response
        .setStatus(201)
        .set(
          'Location',
  ...
  async getById(request, response, next) {
    try {
      ...
      let entity = await this._entityService.getById(id);
      entity = Object.assign({}, entity);
      if (entity.userId !== request.authenticatedUserId) {
  ...
  async delete(request, response, next) {
    ...
    try {
      ...
      console.log(id, request.authenticatedUserId, entity, 'deleting');
      response.json(entity);
    } catch (error) {
      console.log(error);
    }
